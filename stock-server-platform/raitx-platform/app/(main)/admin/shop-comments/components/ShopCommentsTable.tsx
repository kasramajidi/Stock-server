...
import type { ShopCommentItem } from "../lib/shop-comments-api";

interface ShopCommentsTableProps {
  comments: ShopCommentItem[];
  onDelete: (idshop: number | string, id: number | string) => Promise<void>;
  deletingId: string | null;
}

export default function ShopCommentsTable({
  comments,
  onDelete,
  deletingId,
}: ShopCommentsTableProps) {
  const rowKey = (c: ShopCommentItem) => `${c.idshop}-${c.id}`;

  const handleDelete = async (c: ShopCommentItem) => {
    if (!confirm("آیا از حذف این نظر اطمینان دارید؟")) return;
    await onDelete(c.idshop, c.id);
  };
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              <th className="text-right py-3 px-4 font-medium text-gray-700">محصول (idshop)</th>
          ...
          <tbody>
            {comments.length === 0 ? (
              ...
              comments.map((c) => {
                ...
                const isPublished = (c.status ?? "").toLowerCase() === "published" || c.status === "1";
                return (
                  <tr key={key} className="border-b border-gray-100 hover:bg-gray-50/50">
                    <td className="py-3 px-4 text-gray-700">
                      {c.productTitle ? `${c.productTitle} (${c.idshop})` : c.idshop}
                    </td>
