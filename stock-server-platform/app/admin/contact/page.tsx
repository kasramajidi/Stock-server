...
import { MessageSquare, RefreshCw, ChevronDown, ChevronUp, Check, X, Trash2 } from "lucide-react";
...
export default function AdminContactPage() {
  ...
  const load = () => {
    ...
    adminFetch<{ inquiries?: Inquiry[] }>("/api/contact")
      .then(({ data, ok }) => {
        ...
        else setError("خطا در بارگذاری انتقادات و پیشنهادات.");
      })
  ...
  const deleteInquiry = async (id: string, fullName: string) => {
    if (!confirm(`آیا از حذف پیام «${fullName}» مطمئن هستید؟`)) return;
    setActionLoading(id);
    try {
      ...
      <div className="max-w-5xl mx-auto w-full min-w-0" style={{ animation: "adminFadeIn 0.4s ease-out" }}>
        <div className="flex flex-wrap items-center justify-between gap-3 sm:gap-4 mb-4 sm:mb-6">
          <div className="min-w-0">
            <h1 className="text-xl sm:text-2xl font-bold text-slate-800 dark:text-slate-100">انتقادات و پیشنهادات</h1>
            <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">پرسش‌های ارسالی و پاسخ به کاربران</p>
          </div>
          <button
            type="button"
            onClick={load}
              ...
              >
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-slate-800 dark:text-slate-100">{inq.fullName}</p>
                      <p className="text-xs text-slate-500 mt-0.5">{inq.email}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <span
                      className={`rounded-full px-2.5 py-1 text-xs font-medium ${statusColors[inq.status] ?? "bg-slate-400/50 dark:bg-slate-600/50 text-slate-600 dark:text-slate-400"}`}
                    >
                    ...
                    <button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        deleteInquiry(inq.id, inq.fullName);
                      }}
                  ...
                  <div className="border-t border-slate-300 dark:border-slate-700 bg-slate-100 dark:bg-slate-800/50 p-4 space-y-4">
                    ...
                    {inq.adminResponse && (
                      <p className="text-sm text-slate-500 dark:text-slate-400">
                        <span className="text-slate-600 dark:text-slate-500">پاسخ ادمین:</span> {inq.adminResponse}
                      </p>
                    )}
                    {replyForm?.id === inq.id ? (
                      <div className="space-y-3">
                        <textarea
                          ...
                          className="w-full rounded-lg border border-slate-400 dark:border-slate-600 bg-slate-100 dark:bg-slate-700/50 px-3 py-2 text-sm text-slate-800 dark:text-slate-200 min-h-[100px] focus:border-cyan-500 focus:outline-none"
