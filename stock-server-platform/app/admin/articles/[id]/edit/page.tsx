...
export default function AdminEditArticlePage() {
  ...
  const [form, setForm] = useState<ArticleForm & { tagsInput: string }>({
    title: "",
    image: "",
    category: "",
    excerpt: "",
  ...
  useEffect(() => {
    if (!id) return;
    setFetching(true);
    adminFetch<{ article?: ArticleForm }>(`/api/articles/${id}`)
      .then(({ data, ok }) => {
        if (ok && data?.article) {
          ...
          setForm({
            ...
            tagsInput: Array.isArray(a.tags) ? a.tags.join("، ") : "",
          });
        } else setError("مقاله یافت نشد.");
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(Array.isArray(data.errors) && data.errors[0] ? data.errors[0] : "خطا در بروزرسانی مقاله.");
        return;
      }
