...
import { FileText, RefreshCw, ChevronDown, ChevronUp, Check, X } from "lucide-react";
...
export default function AdminCommentsPage() {
  ...
  const load = () => {
    setLoading(true);
    setError(null);
    adminFetch<{ comments?: Comment[] }>("/api/comments")
      .then(({ data, ok }) => {
        if (ok && data?.comments) setList(data.comments);
        else setError("خطا در بارگذاری کامنت‌ها.");
      })
    ...
    <div className="p-6 md:p-8 lg:p-10">
      <div className="max-w-5xl mx-auto" style={{ animation: "adminFadeIn 0.4s ease-out" }}>
        <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
          <div>
            <h1 className="text-2xl font-bold text-slate-100">کامنت مقالات</h1>
            <p className="text-slate-400 text-sm mt-1">تایید، رد و پاسخ به کامنت‌ها</p>
          </div>
          <button
            type="button"
            onClick={load}
        ...
        {loading && list.length === 0 ? (
          ...
          <div className="space-y-3">
            {list.map((c, i) => (
              <div
                key={c.id}
                className="rounded-xl border border-slate-700 bg-slate-800/30 overflow-hidden transition-all duration-200 hover:border-slate-600"
                style={{
              ...
              >
                <div
                  className="flex flex-wrap items-start justify-between gap-3 p-4 cursor-pointer hover:bg-slate-700/20"
                  ...
                  <div className="flex items-start gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-slate-500 mt-1">
                        {c.user?.fullName} · {c.article?.title}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <span
                      className={`rounded-full px-2.5 py-1 text-xs font-medium ${statusColors[c.status] ?? "bg-slate-600/50 text-slate-400"}`}
                    >
                ...
                {expandedId === c.id && (
                  <div className="border-t border-slate-700 bg-slate-800/50 p-4 space-y-4">
                    ...
                    {c.adminReply && (
                      <p className="text-sm text-slate-400 border-r-2 border-cyan-500/50 pr-3">
                    ...
                    {replyForm?.id === c.id ? (
                      <div className="space-y-3">
                        <textarea
                          ...
                          className="w-full rounded-lg border border-slate-600 bg-slate-700/50 px-3 py-2 text-sm text-slate-200 min-h-[80px] focus:border-cyan-500 focus:outline-none"
