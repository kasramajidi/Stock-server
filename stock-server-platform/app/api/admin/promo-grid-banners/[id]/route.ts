...
/**
 * PATCH /api/admin/promo-grid-banners/[id] — ویرایش بنر
 */
export async function PATCH(
  ...
  try {
    ...
    const banner = await prisma.promoGridBanner.update({
      where: { id },
      data,
    });
    return NextResponse.json({ success: true, banner });
  } catch (e) {
    console.error("Admin promo grid banner PATCH error:", e);
    return NextResponse.json(
...
/**
 * DELETE /api/admin/promo-grid-banners/[id] — حذف بنر
 */
export async function DELETE(
  ...
  const { id } = await params;
  try {
    await prisma.promoGridBanner.delete({
      where: { id },
    });
    return NextResponse.json({ success: true });
  } catch (e) {
    console.error("Admin promo grid banner DELETE error:", e);
    return NextResponse.json(
